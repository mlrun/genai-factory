...
>(
  (
    ...
    const [pinned, setPinned] = React.useState(
      localStorage.getItem('isNavbarPinned') === 'true',
    );
    ...
    const contextValue = React.useMemo<SidebarContext>(
      ...
      [state, open, setOpen, toggleSidebar, pinned, togglePin],
    );
...
const Sidebar = React.forwardRef<SidebarElement, SidebarProps>(
  (
    ...
    const { open, pinned, setOpen, state, togglePin } = useSidebar();
    ...
    return (
      ...
      >
        ...
        <div
          className={cn(
            'fixed h-svh w-[var(--sidebar-width)] bg-transparent transition-[width] duration-300 ease-linear',
        ...
        <div
          className={cn(
            'inset-y-0 z-10 hidden h-svh w-[var(--sidebar-width)] transition-[left,right,width] duration-300 ease-linear md:flex',
            side === 'left'
        ...
        >
          ...
          >
            ...
            {open && (
              <Button
                variant="outline"
                className="absolute top-2 left-full w-fit h-fit px-1 border-0 rounded-l-none border-l-0
                text-white bg-sidebar
...
>(({ asChild = false, className, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        'flex h-9 shrink-0 items-center rounded-md px-2.5 text-xs font-medium text-sidebar-foreground/70 outline-none ring-sidebar-ring transition-[margin,opa] duration-300 ease-linear focus-visible:ring-2 [&>svg]:size-5 [&>svg]:shrink-0',
...
const sidebarMenuButtonVariants = cva(
  'peer/menu-button flex flex-1 w-full items-center gap-2 overflow-hidden rounded-md text-left text-sm outline-none ring-sidebar-ring transition-[width,height,margin] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:mr-2 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-active-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground mx-1.5 whitespace-nowrap [&>svg]:size-5 [&>svg]:shrink-0',
  {
    variants: {
      variant: {
